...
앱스크립트 편집기의 ▶ 실행 버튼은 **함수에 인자를 넘길 수 없습니다.** 그래서 배포 주소는
`Code.gs` 상단 상수에 적어 두고, 인자 없는 함수 하나만 실행하면 됩니다.

```js
var PLATFORM_URL_DEFAULT = 'https://25gps.netlify.app';  // ← 주소가 바뀌면 이 줄만 수정
```

편집기 함수 선택창에서 **`setupPull`** 하나만 고르고 ▶ 실행하면 끝입니다.
(주소 저장 → 1분 주기 트리거 등록 → 즉시 1회 동기화 → 상태 출력)

실행 로그에서 아래 두 줄을 확인하세요.
...
`플랫폼에 올라간 것: 0개` 이면 아직 원장이 전달되지 않은 것입니다. 1분 뒤 `checkPull()` 로 다시 확인하세요.
...
| 키 | 값 | 필수 | 설명 |
|---|---|---|---|
| `SOLAPI_API_KEY` | 솔라피 콘솔의 API Key | ✅ | |
| `SOLAPI_API_SECRET` | 솔라피 콘솔의 API Secret | ✅ | **절대 저장소에 커밋하지 마세요** |
| `SOLAPI_SENDER` | 사전 등록된 발신번호 | ✅ | 미설정이면 자동으로 `log` 모드 |
| `SMS_MODE` | `log` 또는 `live` | | 미지정이면 키 유무로 자동 판단 |
| `SMS_ALLOWLIST` | 쉼표 구분 번호 목록 | | 지정하면 **이 번호로만** 발송 |
| `SMS_OPS_NUMBER` | 본부 담당자 번호 | | 질문 접수·미응답 알림 수신 |

### 안전한 전환 순서

1. `SMS_MODE=log` 로 배포해 발송 없이 동작만 확인합니다. 관리자 → 문자 상태에서 기록을 봅니다.
2. `SMS_ALLOWLIST` 에 **본인 번호만** 넣고 `SMS_MODE=live` 로 바꿔 테스트 1건을 받아 봅니다.
3. 정상 확인 후 `SMS_ALLOWLIST` 를 비우면 전체 발송이 열립니다.

> `SMS_ALLOWLIST` 를 비우는 순간 전국 경영주에게 실제 문자가 나갑니다. 마지막에 하세요.

### 안전장치

- 광고성 코드(`PRE_NOTIFY`·`NUDGE`·`COUPON`)는 야간 21~08시(KST) 자동 차단
